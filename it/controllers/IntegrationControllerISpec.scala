...
class IntegrationControllerISpec extends ServerBaseISpec with BeforeAndAfterEach with IntegrationCatalogueConnectorStub with ApiDetailTestData {
  ...
  trait Setup {
    ...
    def validDeleteIntegrationByPlatformRequest(platform: String): FakeRequest[AnyContentAsEmpty.type] = {
      FakeRequest(Helpers.DELETE, s"/integration-catalogue-admin-frontend/services/integrations$platform")
        .withHeaders(masterKeyHeader: _*)
    }
  ...
  "IntegrationController" when {
    ...
    "DELETE /services/integrations?platform=CORE_IF" should {

      "respond with 204 when deletion successful" in new Setup {
        primeIntegrationCatalogueServiceDeleteByPlatform(s"?platform=${coreIfPlatform.toString}", NO_CONTENT)

        val response: Future[Result] = route(app, validDeleteIntegrationByPlatformRequest(coreIfPlatform.toString)).get
        status(response) mustBe NO_CONTENT
      }
    }
  }
